...
// Hand   The hand is a basic trackable object through the LeapMotionDevice.
...
// Gesture  A Frame can have a list of Gestures that are starting, updating, or ending.
...
// Pointable   A Finger or Tool can be a Pointable, as well as pointables themselves.
...
// Finger   A Frame can have a list of Fingers, which are structured like Pointables
type Finger struct {
	ID int `json:"id"`
	*Pointable
}

// Tool   A Frame can have a list of Tools, which are structured like Pointables
type Tool struct {
	ID int `json:"id"`
	*Pointable
}

// Frame This struct represents each Frame presented on the LeapMotionDevice WebSocket.
